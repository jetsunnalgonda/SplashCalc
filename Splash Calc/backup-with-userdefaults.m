...
{
    NSLog(@"view will layout subviews (graph view controller");
    [self adjustSize];
}
...
- (void)adjustSize
{
    ...
    self.view.frame = CGRectMake(self.view.bounds.origin.x, self.view.bounds.origin.y + navigationHeight + statusBarHeight,
                                 self.view.bounds.size.width, self.view.bounds.size.height - navigationHeight - statusBarHeight);
    
    CGFloat aspectRatio = self.view.frame.size.width / self.view.frame.size.height;
    
    //    NSLog(@"self.view.frame = %@", NSStringFromCGRect(self.view.frame));
    //    NSLog(@"self.view.bounds = %@", NSStringFromCGRect(self.view.bounds));
    //    NSLog(@"super.view.frame = %@", NSStringFromCGRect(super.view.frame));
    //    NSLog(@"super.view.bounds = %@", NSStringFromCGRect(super.view.bounds));
    
    
    // Set starting zoom level and maximum zoom level
    self.plotSpace.xRange = [CPTPlotRange
                             plotRangeWithLocation:@(-2.0 * aspectRatio)
                             length:@(4.0 * aspectRatio)];
...
- (void)viewDidLoad
{
    ...
    //    self.brain.program = (NSArray *)[settings objectForKey:@"program"];
    //
...
- (void)loadGraph
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (int i = 0; i <= numberOfRecords; i++)
        {
            ...
            CGFloat xValue = (indexFloat - numberOfRecords / 2) * precision;
...
-(NSNumber *)numberForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index
{
    ...
    CGFloat numberOfRecords = (CGFloat)plotLength / self.precision;
