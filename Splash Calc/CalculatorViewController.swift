...
@objc class CalculatorViewController: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate
{
    // MARK: - Outlets
...
//            label.sizeToFit()
//            let width = min(label.frame.size.width, 150)
//            let size = CGSizeMake(width, label.frame.size.height)
//            
//            let shouldHide = errorText == "" ? true : false
//            
//            println("shouldHide = \(shouldHide)")
//            println("errorLabel.text = \(errorLabel.text)")
//            if shouldHide {
//                println("hiding")
//                UIView.animateWithDuration(0.6, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 1.0, options: nil, animations: { [unowned self] () -> Void in
//                    self.errorLabel.frame.size.height = 0.0
//                    self.errorLabel.frame.size.width = 0.0
//                    
//                    }, completion: {
//                        [unowned self] (value: Bool) in self.errorLabel.hidden = true
//                        self.errorLabel.frame.size = size
//                    } )
//            } else {
//                println("showing")
//                errorLabel.text = " ⚠️ " + errorText
//                errorLabel.frame.size = CGSizeZero
//                errorLabel.hidden = false
//                UIView.animateWithDuration(0.6, delay: 0.0, usingSpringWithDamping: 0.6, initialSpringVelocity: 1.0, options: nil, animations: { [unowned self] () -> Void in
//                    self.errorLabel.frame.size = size
//                    }, completion: nil )
//            }
//        }
        ...
        switch title {
            ...
            case "AC":
                var refreshAlert = UIAlertController(
                    title: "Memory Clear",
                    message: "Calculator memory will be cleared.",
...
//    }
    ...
    {
//        println("adjusting scroll view")
//        println("history.bounds.size = \(history.bounds.size)")
//        println("historyScrollView.contentSize = \(historyScrollView.contentSize)")
//        println("historyHorizontalSpace.constant = \(historyHorizontalSpace.constant)")
...
//            historyScrollView.contentOffset = CGPointMake(historyScrollView.frame.size.width, historyScrollView.frame.size.height)
        ...
        //historyScrollView.contentSize.width = history.bounds.size.width
        
//        println("historyScrollView.contentSize = \(historyScrollView.contentSize)")
//        println(" ")
//        historyScrollView.scrollRectToVisible(historyScrollView.frame, animated: true)
        
//        let frame: CGRect
//        frame.origin = historyScrollView.frame.origin
...
//        println("visibleRect = \(visibleRect)")
//
//        println("history.contentSize = \(history.contentSize)")
//        println("historyCenterX.constant = \(historyCenterX.constant)")
//        println("historyScrollView.contentOffset = \(historyScrollView.contentOffset)")
//        println("historyHorizontalSpace.constant = \(historyHorizontalSpace.constant)")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let identifier = segue.identifier {
            switch identifier {
            case Segue.Graph:
                if let gvc: GraphViewController = destination as? GraphViewController {
                    if !rpnMode {
                        execute()
                        gvc.title = "".join(displayStack)
                    }
                    if rpnMode {
                        gvc.title = brain.description == "?" ? "Graph" : brain.description.components(separatedBy: ", ").last
                        gvc.program = brain.program as! [AnyObject]
                    }
                    
                    checkFreshStart()
                    gvc.stack = brain.parseStack
                    gvc.values = brain.variableValues
                    println("brain.parseStack = \(brain.parseStack)")
                    for (key, value) in brain.variableValues {
                        brain.variableValuesNS[key] = brain.variableValues
                    }
